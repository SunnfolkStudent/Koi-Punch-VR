...
namespace VHierarchy.Libs
{
    ...
    public static class VGUI
    {
        ...
        public static void MarkInteractive(this Rect rect) // does a dummy toggle to trigger repaint on mouse enter/leave
        {
            if (curEvent.isMouse) return;
            // if (!curEvent.isRepaint) return;

            var prev = GUI.color;
            GUI.color = Color.clear;

            GUI.Toggle(rect, false, "");
            // GUI.Button(rect, ""); // won't allow pressed buttons in the rect to be blue, for some reason
